         ...
         <div class="row">
            <div class="col-md-12">
               {{--<ol class="breadcrumb text-right text-black mb-0 mt-40">
                  ...
                  <li><a href="{{lang_url('')}}">@t('Video')</a></li>
                  <li class="active text-gray-silver">@t('Online School Details')</li>
               </ol>--}}
               <h2 class="title text-white">@t('Online School Details')</h2>
            </div>
         </div>
      </div>
   </div>
...
<section class="divider bg-white">
   <div class="container" style="padding-top: 50px">
       <div class="row <?= (request()->segments()[0] == "ar" ? "rtl" : "ltr") ?> mb-20" >
           <div class="col-sm-12">
               <?php $course = \App\Course::find($chapter->chapters->course_id) ?>
               <?php $school = \App\School::find($course->school_id) ?>
               <h3 class="color-theme-green"><?= $school->name ?> / <?= $course->name ?> / <?= $chapter->name ?></h3>
           </div>
      ...
      <div class="row">
         <div class="col-md-3 col-sm-3 col-xs-12">
            <div class="vertical-tab">
               <ul class="nav nav-tabs">
                  ...
                  <li class="active"><a href="{{ lang_url('schools') }}"><img src="/frontend/_assets/images/icon-4.png" class="img-responsive" alt="icon-4"/>@t('Electronic School')</a></li>
         ...
         <div class="col-md-9 col-sm-9-col-xs-12">
            <?php
               ...
               ?>
            <?php if ($chapterCompleted): ?>
                <?php /*if ($skipTest > 0): */?>
                <?php $exam = Exams::where("chapter_id", "=", $chapter->chapter_id)->first(); ?>
                <?php if ($exam) : ?>
                  ...
                  <div class="p-4 mb-5 videos_design_li">
                     <?php
                        ...
                        ?>
                     <?php if (count((array)$chapterCompleted) < 1): ?>
                     <!-- Model Box For Test -->
                     ...
                     <div class="row" style="margin-bottom:10px;">
                        ...
                        <div class="col-xs-10">
                            <?php
                            $exam = Exams::where("chapter_id", "=", $chapter->chapter_id)->first();
                            $questions = \App\TestQuestion::where("exam_id", "=", $exam->id)->count();
                            ?>
                           <div><?= $questions ?></div>
                     ...
                     <!-- /.row -->
                     <?php $result = \App\TestResult::where(["exam_id" => $exam->id, "user_id" => Auth::user()->id])->first() ?>
                     <?php if (!$result || $result->result != "Passed") : ?>                      
                        <?php if ($chapterExamQuestions->first()) : ?>
                            <button class="btn btn-success"><a href="{{ lang_url('chapters/'.$chapter->id.'/test/serve') }}" class="text-white">
                        <?php endif; ?>
                      @t('Start Test')</a></button>
                 		<?php else : ?>
                 			<p style="font-size: 20px" class="color-theme-green">@t('Exam is successfully taken')!</p>
                 		<?php endif; ?>
                  </div>
                  <?php endif; ?>
                <?php endif; ?>
         ...
         <div class="col-md-9 col-sm-9 col-xs-12">
            <div class="tab-content">
               ...
               <div class="tab-pane fade in active" id="tab5">
                  <div class="row mb-10">
                     <div class="col-md-12">
                        <h2 class="color-theme-green">@t('Chapter Details')</h2>
                     </div>
                  </div>
                  <div class="row">
                     <div class="col-md-12">
                  ...
                  <div class="row mt-30">
                     <div class="col-md-12 col-sm-12 col-xs-12">
                        ...
                        <div class="row main_comment_{{ $video->id }} mainVideosDiv" data-videonative-id="{{ $video->id }}" id="video_list_{{ $video->id }}">
                           ...
                           <?php if ($decodedVideo): ?>
                           <h4 class="color-dark-green"><a class="color-dark-green" href="\public\storage\{{ $decodedVideo[0]->download_link }}" download>{{ $video->name }} <span style="font-size: 1.2em;"><i class="fa fa-check-circle" style="color:{{ $videoWatchedSingle ? '#3c763d' : '#000' }};" aria-hidden="true"></i></a></h4>
                           ...
                           <div class="fluid-video-wrapper">
                              ...
                              <?php $videoWatched = User_access::where([['user_id', Auth::user()->id], ['object_type', 'video'], ['object_id', $last_video_id], ['status', 'watched']])->first();
                                 ?>
                              <?php if ($decodedVideo && $decodedVideo[0]) : ?>
                                    ...
                                    <div class="media comment-author">
                                       ...
                                       <div class="col-xs-10 media-body bg-lighter p-10">
                                          <div class="comment-section">
                                             <p>{{ $comment->comment }}</p>
                                             <button class="rep-btn comment_reply_button rep-btn pull-right m-0" data-coach_id="{{ $video->user_id }}" data-video_id="{{ $video->id }}" data-parent_id="{{ $comment->id }}">@t('Reply')</button>
...
<?php $send = @t('send') ?>
<?php $plc = @t('Write your reply here') ?>
<?php $r = @t('Reply') ?>
<script>
       ...
       $(document).on('click', '.comment_reply_button', function(e) {
           ...
           var $this = $(this),
               ...
               replyForm = `<div class="rep-btn-section float-left col-11">
                ...
                @csrf
                    <textarea name="current_user_comment" class="user_cmnt w-100 form-control" placeholder="<?= $plc ?>" style="resize: none;"></textarea>
                     <input type="hidden" name="video_id" value="` + video_id + `">
                     <input type="hidden" name="coach_id" value="` + coach_id + `">
                     <input type="hidden" name="parent_id" value="` + parent_id + `">
                     <input type="hidden" name="user_id" value="{{ Auth::user()->id }}">
                     <div style="width: 100%; float: left;">
                       <button type="submit" class="btn btn-success comment"><?= $send ?></button>
                     </div>
                  </form>
                </div>`;
                                                       ...
                                                       <div class="media comment-author">
                                                         ...
                                                         <div class="col-xs-10 media-body bg-lighter p-10">
                                                           <div class="comment-section">
                                                             ...
                                                             <button class="rep-btn comment_reply_button rep-btn pull-right m-0" data-coach_id="`+coach_idAppend+`" data-video_id="` + video_id + `" data-parent_id="` + response + `"><?= $r ?></button>
                                                           </div>
                                                         </div>
                                                     </div>
                                                     </li>`;
