...
class IndexController extends Controller
{
	...
	public function school_detail($school_id, $subscriptions_id)
	{
		if (Auth::check()) {
			...
			if ($schoolAccess) {
                ...
                ->whereHas('courses', function ($query) use ($school_id) {
		    		$query->where('school_id', $school_id);
		    	})->where([['lang', $this->langCode], ['status', 'active']])->orderBy('courses.sort', 'ASC')->get();
			...
			if ($role_id == '1') {
		        ...
		        if ($foundRequest) {
        			...
        			if (!empty($last_id)) {
        				...
        				return redirect()->back()->with('message', t('Thank you for your interest to be a coach. Your request has been sent successfully! We will review your application and we will contact you if approved!'));
			        }
				}
			}
		} else {
	...
	public function test_answer(Request $request)
	{
		...
		$insertAnswer = [
            ...
            $prevAnswers = UserAnswer::where(["exam_id" => $exam_id, "user_id" => $user_id]);

            if ($prevAnswers) {
                foreach ($prevAnswers as $item) {
                    $item->delele();
                }
            }
	...
	public function eventDetail($event_id)
	{
		...
		$eventNative = EventsNative::with('events')->where(
		    [
		        ['event_id', $event_id],
                ['lang', $this->langCode],
                ['status', 'active']
