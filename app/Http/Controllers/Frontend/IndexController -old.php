...
class IndexController extends Controller
{
    ...
    public function register()
    {
        ...
        if (Auth::check()) {
            ...
            return view('frontend.register', compact('title', 'countries'));
        }
    }
    ...
    public function buy_product($product_native_id)
    {
        ...
        if (Auth::check() && !empty($productNative)) {

            if (!empty($productNative)) {
    ...
    {    
        ...
        $discount = ($product_price * $qty_input + $tax_price + $shipping_price) - $total_price;
        if ($user) {
            $user_id = $user->id;
        } else {
            $user_id = NULL;
        ...
        $adminSubject = 'New order was made';
		    	...
		    	<div style="text-align: left;padding-left: 20px;padding-top: 50px;padding-bottom: 30px;">
			    	...
			    	<p>
			    		<ul style="list-style: none;">
			    			<li>Product Name: '.$productName->name.'</li>
			    			<li>Price: '.$product_price.'</li>
                            <li>Discount: '.$discount.'</li>
                            <li>Tax: '.$tax_price.'</li>
                            <li>Shipping fee: '.$shipping_price.'</li>
		    	...
		    	<div style="text-align: left;padding-left: 20px;padding-top: 50px;padding-bottom: 30px;">
			    	...
			    	<p>
			    		<ul style="list-style: none;">
			    			...
			    			<li>Last Name: '.$user_last_name.'</li>
        ...
        $dataAdmin = array( 'email' => $adminEmail, 'subject' => $adminSubject, 'message' => $admin_msg_template);
        //User
        Mail::send([], $data, function ($m) use($data) {
            $m->to($data['email'])->subject($data['subject'])->setBody($data['message'], 'text/html');
        });
    ...
    public function plans_pricing()
    {
        ...
        return view('frontend.plans_pricing', compact('title', 'allPlans', 'isAvailable'));
    }
    ...
    public function school_detail($school_id, $subscriptions_id)
    {
        if (Auth::check()) {
            ...
            if ($schoolAccess) {
                ...
                $courseNative = CoursesNative::with('courses')
                    ->leftJoin("courses", "courses_natives.course_id", "=", "courses.id")
                    ->where([
                        ['lang', $this->langCode],
                        ['status', 'active'],
                        ['school_id', $school_id]
                    ])->orderBy('courses.sort', "ASC")->get();

                if (!empty($schoolNative)) {
    ...
    public function chapter_detail($chapter_id)
    {
        ...
        if (Auth::check()) {
            ...
            $chapterExamQuestions = DB::table('exam')
                    ...
                    ->where([['exam.chapter_id', $chapter->chapters->id], ['exam.status', 'active'], ['test_questions.question_status', 'active']])
                    ->get();

            if (!empty($chapter)) {
                $last_video_id = Video::where("chapter_id", $chapter_id)->orderBy("id", "DESC")->first()->id;
                $title = $chapter->name;
                return view('frontend.chapter_detail', compact('title', 'chapter', 'userAgent', "last_video_id", "chapterExamQuestions"));
    ...
    public function buy_plan(Request $request)
    {
        ...
        if ($method && isset($_SESSION['plan_id']) && $_SESSION['plan_id'] != $_POST['plan_id']) {
            $_SESSION['plan_id'] = $_POST['plan_id'];
        } else if ($method && ( !isset($_SESSION['plan_id']) || !$_SESSION['plan_id'] )) {
            $_SESSION['plan_id'] = $_POST['plan_id'];
        }
    
        $planId = $_SESSION['plan_id'];
        $plan = SchoolPlan::find($planId);
        $title = 'Buy Plan';
        $price = $plan->price;
        if (!isset($_SESSION['plan_no'])) {
            $_SESSION['plan_no'] = $request->no; 
        }
        $no = $_SESSION['plan_no'];
        $duration = $plan->duration;        
        $plan_name = $plan->name;
        $schoolNumber = $plan->schools_number;
        $maxSchools = $plan->schools_number;
        $priority = $plan->priority;
        $userID = \Auth::user()->id;
        $userPackages = UserSubscription::where("user_id", "=", $userID)->get();
        $schoolIDs = [];
        $schoolPlans = \App\SchoolPlanSchools::all();

        $schoolNative = SchoolsNative::with('schools')
            ->leftJoin("school_plan_schools", "schools_natives.school_id", "=", "school_plan_schools.school_id")
            ->where([
                ['status', 'active'],
                ['plan_id', $planId]
            ])
        ...
        return view('frontend.plan_purchase', compact('maxSchools', 'schoolIDs', 'title', 'schoolNative', 'no', 'price', 'plan_name', 'planId', 'duration', 'schoolNumber', 'isAvailable'));
    }
    ...
    public function buy_plan_school(Request $request)
    {
        ...
        if (gettype($school) == "array") {
            foreach ($school as $id) {
                $currentSchoolPlan = UserSubscription::where([
                    "user_id" => $user_id,
                    "school_id" => $id
                ])->first();
                if ($currentSchoolPlan) {
                    $currentSchoolPlan->package_name = $plan_name;
                    $currentSchoolPlan->package_start_date = $package_start_date;
                    $currentSchoolPlan->package_end_date = $package_end_date;
                    $currentSchoolPlan->package_price = $price;
                    $currentSchoolPlan->save();
                    $inserted = true;
                } else {
            ...
            $oldModel = UserSubscription::where([
                "user_id" => $user_id,
                "school_id" => $school
            ])->first();
            if ($oldModel) {
                $oldModel->update($userData);
                $inserted = true;
            } else {
                $inserted = UserSubscription::insert($userData);
            }
    ...
    public function profile()
    {
        ...
        if (Auth::check()) {
            ...
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_URL, "https://api.teleport.org/api/countries/");
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            $data = curl_exec($ch);
            curl_close($ch);
            $data = json_decode($data, true);
            $countriesArray = $data['_links']['country:items'];
            $countries = [];
            foreach ($countriesArray as $c) {
                array_push($countries, $c['name']);
            }

            return view('frontend.profile', compact('title', 'UserTbl', 'countries'));
        } else {
            Session::put('url.intended', lang_url('profile'));
            return redirect(lang_url('userlogin'));
        }
    ...
    public function loadCities($id)
    {
        ...
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, "https://api.teleport.org/api/cities/?search=".$name);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $data = curl_exec($ch);
        curl_close($ch);
        $data = json_decode($data, true);
        $cityInfoArray = $data['_embedded']['city:search-results'];
        $cityArray = [];
        foreach ($cityInfoArray as $city) {
            $arrayName = explode(",", $city['matching_full_name']);
            $cityArray[] = $arrayName[0];
        }
    ...
    public function update_my_data(Request $request)
    {
        ...
        $fin_market = $request->input('fin_market');
    ...
    public function my_purchases()
    {
        ...
        if (Auth::check()) {
            ...
            return view('frontend.my_purchases', compact('title', 'all_purchases', 'langCode'));
        } else {
    ...
    public function activity_detail($subscription_id)
    {
        ...
        if (Auth::check()) {
            ...
            if ($training_activity) {
                $additionalRecords = \App\Event::where("parent_id", "=", $training_activity->event_id)->get();
                return view('frontend.activity_detail', compact('title', 'training_activity', 'additionalRecords'));
            } else {
    ...
    public function email_subscribe_user(Request $request)
    {
        ...
        if (filter_var($email, FILTER_VALIDATE_EMAIL) ) {
            ...
            $uns = t('Unsubscribe here');
            if ($emailFound) {
                $unsubscribe = '<a style="font-size: 10px;" class="btn btn-primary btn-sm" href="email_unsubscribed/'.$email.'">'.$uns.' </a>';
            ...
            if ($role_id == '1') {
                ...
                if ($foundRequest) {
                    ...
                    if (!empty($last_id)) {
                        ...
                        return redirect()->back()->with('message', 'Thank you for your interest to be a coach. Your request has been sent successfully! We will review your application and we will contact you if approved!');
                    }
                }
            }
        } else {
    ...
    public function test_answer(Request $request)
    {
        ...
        if ($ansCheck) {
            ...
            $questionsCount = TestQuestion::where("exam_id", "=", $exam_id)->count();

            $perc_per_answer = 100 / $questionsCount;

            $answerInserted = UserAnswer::insert($insertAnswer);
            if ($answerInserted) {

                $percentage = $perc_per_answer;

                if ($answer_status == TRUE) {
                    $initPercentage = (int)$initPercentage + (int)$percentage;
                }

                if ($initPercentage == 99) {
                    $initPercentage = 100;
                }

                if ((int)$min_passing <= (int)$initPercentage ) {
                    $overall_result = t('Passed');
                    $dbResult = "Passed";
                } else {
                    $overall_result = t('Not Passed');
                    $dbResult = "Not passed";
                }

                if ($remainingQuestions == 0) {

                    $answCount = DB::table("test_answers_users")->where('exam_id', '=', $exam_id)
                        ->where(function ($query) use ($user_id) {
                            $query->where('user_id', '=', $user_id);
                    })
                    ->count();
                    if ($questionsCount < $answCount) {
                        DB::table("test_answers_users")->where('exam_id', '=', $exam_id)
                        ->where(function ($query) use ($user_id) {
                            $query->where('user_id', '=', $user_id);
                        })
                    ...
                    /*$prevResults = TestResult::where("exam_id", "=", $exam_id);*/
                    ...
                    if ($resultInserted) {
                        $chapter_native = \App\ChaptersNative::find($chapter_native_id);
                        $insertChapterAccess = [
                            'user_id' => $user_id,
                            'object_id' => $chapter_native->chapter_id,
                            'object_type' => 'test',
                            'status' => $dbResult,
                            'created_at' => Carbon::now()
                        ];
                ...
                $result = [
                    ...
                    'currentResult' => $answer_status == true ? 1 : 0
                ];
    ...
    public function all_tests()
    {
        $title = 'All Tests';
        if (Auth::check()) {
            $allTests = TestResult::with('exam')->where('user_id', Auth::user()->id)->orderBy("id", "DESC")/*->groupBy('test_results.exam_id')*/->get();
    ...
    public function payment_course($event_id, $subscriptions_id)
    {
        ...
        if (!empty($eventNative) && !empty($subscription)) {
        ...
        if (!$foundCode) {
            ...
            if ($object_type == '7') {
                ...
                if ($event) {
                    ...
                    if ($foundCode->user_id == null || $foundCode->user_id == Auth::user()->id) {
    ...
    public function deleteEventUser(Request $request) 
    {
        $id = $request->input('id');
        $event_id = $request->input('event_id');
        $subRecord = DB::table('course_subscriptions')->where(['user_id' => $id, 'event_id' => $event_id])->delete();
    }
    ...
    public function listings_bill($id, $type)
    {
        ...
        if ($type == 1) {
            ...
            $order = UserSubscription::find($id);
        } else {
            $order = EventsNative::find($id);
        }

        $lang = request()->segments()[0];

        $dirPath = public_path()."/storage/bills/".session()->getId();
        if (!file_exists($dirPath)) {
            mkdir($dirPath, 0777, true);
        }
        $pdfFilePath = $dirPath . "/" . "bill.pdf";
        $pdf = PDF::loadView('pdf.document', ['data' => $order->getAttributes(), 'type' => $type, 'lang' => $lang]);
        $pdf->save($pdfFilePath);

        return view('frontend.bill', compact('title', "order", "type"));
    ...
    {   
        $query = $_POST['q'];
        return User::select("id", "name as text")->where("name", "LIKE", "%" . $query . "%")->get()->toArray();
    }
    ...
    public function copy($id, $className, $path)
    {
        $class = "\App\\$className";
        $model = $class::find($id);
        $newModel = $model->replicate();
        $newModel->save();
        return \redirect("/admin/$path/$newModel->id/edit");
    }
    ...
    {
        $tmpDir = public_path()."/storage/bills/".session()->getId(); 
